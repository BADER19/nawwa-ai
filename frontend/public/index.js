...
function responsive() {
    if (!collapseHeaderItems || !navToggle) return
    if (!isHeaderCollapsed){
        toggleHeader()
    }
    ...
    if (window.innerWidth > RESPONSIVE_WIDTH) {
        ...
        navToggle && navToggle.addEventListener("mouseenter", openNavDropdown)
        navToggle && navToggle.addEventListener("mouseleave", navMouseLeave)

    } else {
        isHeaderCollapsed = true
        navToggle && navToggle.removeEventListener("mouseenter", openNavDropdown)
        navToggle && navToggle.removeEventListener("mouseleave", navMouseLeave)
    }
}
responsive()
...
const demoEl = document.querySelector("#instantviz-demo")
if (demoEl) {
    const promptWindow =  new Prompt("#instantviz-demo")
...
navToggle && navToggle.addEventListener("click", toggleNavDropdown)
navDropdown && navDropdown.addEventListener("mouseleave", closeNavDropdown)
...
if (window.Typed && document.querySelector('#prompts-sample')) {
    const typed = new Typed('#prompts-sample', {
        strings: [
            "Plot y = x^2",
            "What is InstantViz?",
            "Explain photosynthesis",
            "How do I use the InstantViz API?"
        ],
        typeSpeed: 80,
        smartBackspace: true, 
        loop: true,
...
// straightens the slanting image
    if (document.querySelector('#dashboard')) gsap.to("#dashboard", {
